...
def load_graph_dataset(_dataset, shuffle=True):
    ...
    if _dataset == "ba2":
        ...
        dir_path = os.path.dirname(os.path.realpath(__file__))
        path = dir_path + '/pkls/' + "BA-Reg" + '.pkl'
        with open(path, 'rb') as fin:
            adjs, features, labels = pkl.load(fin)
    elif _dataset == "bareg2":
        dir_path = os.path.dirname(os.path.realpath(__file__))
        path = dir_path + '/pkls/' + "BA-Reg2" + '.pkl'
        with open(path, 'rb') as fin:
            adjs, features, labels = pkl.load(fin)
    elif _dataset == "bareg3":
        dir_path = os.path.dirname(os.path.realpath(__file__))
        path = dir_path + '/pkls/' + "BA-Reg3" + '.pkl'
        with open(path, 'rb') as fin:
            adjs, features, labels = pkl.load(fin)
    elif _dataset == "bareg2_2":
        dir_path = os.path.dirname(os.path.realpath(__file__))
        path = dir_path + '/pkls/' + "BA-Reg2_2" + '.pkl'
        with open(path, 'rb') as fin:
            adjs, features, labels = pkl.load(fin)
    elif _dataset == "mutag":
        dir_path = os.path.dirname(os.path.realpath(__file__))
