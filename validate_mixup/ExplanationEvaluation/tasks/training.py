...
def create_gt_data_list(graphs, features, labels, mask):
    """
    Convert the numpy data to torch tensors and save them in a list.
    :params graphs: edge indecs of the graphs
    :params features: features for every node
    ...
    for i in indices:
        ...
        for idx in range(len(edge_index[0])):
            if edge_index[0][idx] >= 20 and edge_index[1][idx] >= 20:
                new_edge_index[0].append(edge_index[0][idx])
                new_edge_index[1].append(edge_index[1][idx])
...
def train_graph(_dataset, _paper, args):
    ...
    val_gt_set = create_gt_data_list(graphs, features, labels, val_mask)

    print(len(val_set), len(val_gt_set))
    train_loader = DataLoader(train_set, batch_size=args.batch_size, shuffle=True)
    val_loader = DataLoader(val_set, batch_size=len(val_set), shuffle=False)
    test_loader = DataLoader(test_set, batch_size=len(test_set), shuffle=False)
    val_gt_loader = DataLoader(val_gt_set, batch_size=len(val_gt_set), shuffle=False)
    ...
    for epoch in range(0, args.epochs):
        ...
        with torch.no_grad():
            ...
            eval_data = next(iter(val_gt_loader))  # Loads all eval ground truth samples
            out, _ = model(eval_data.x, eval_data.edge_index, eval_data.batch)
            val_gt_acc = evaluate(out, eval_data.y)

        print(f"Epoch: {epoch}, train_acc: {train_acc:.4f}, val_acc: {val_acc:.4f}, val_gt_acc: {val_gt_acc:.4f}, "
    ...
    def val_embedding(val_alt_loader):
        eval_data = next(iter(val_alt_loader))
        _, out = model(eval_data.x, eval_data.edge_index, eval_data.batch)
        return out

    val_ebd = val_embedding(val_loader)
    val_ebd_1 = val_embedding(val_gt_loader)

    val_ebd = val_ebd.detach().tolist()
    val_ebd_1 = val_ebd_1.detach().tolist()

    res = [val_ebd, val_ebd_1]
    with open('./results/val_embedding_classification.json', 'w') as f:
