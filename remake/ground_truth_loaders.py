...
from data_utils import preprocess_adj, adj_to_edge_index, load_real_dataset, get_graph_data
...
def load_bareg_dataset_ground_truth(shuffle=True):
    ...
    shuffled_adjs = adjs[shuffled_indices]
    shuffled_edge_index = adj_to_edge_index(shuffled_adjs)
    shuffled_ground_truth = ground_truth[shuffled_indices]
    np_edge_labels = []

    # Obtain the edge labels.
    insert = 20
    skip = 5
    for i in range(len(shuffled_edge_index)):
        edge_index = shuffled_edge_index[i]
        ground_truth = shuffled_ground_truth[i]
...
def load_dataset_ground_truth(_dataset, test_indices=None):
    ...
    """
    if _dataset == "bareg1" or _dataset == "bareg2":
        graph, labels = load_bareg_dataset_ground_truth(_dataset)
