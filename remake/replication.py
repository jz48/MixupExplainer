...
from ground_truth_loaders import load_dataset_ground_truth
...
def run_qualitative_experiment(explainer, indices, labels, config, explanation_labels):
    ...
    for idx in indices:
        graph, expl = explainer.explain(idx)
        plot(graph, expl, explainer.features, labels, idx, config.thres_min, config.thres_snip, config.dataset, config,
             explanation_labels)
...
def replication(config, extension=False, run_qual=True, results_store=True):
    ...
    graphs, features, labels, _, _, test_mask = load_dataset(config.dataset, paper='GNN')
